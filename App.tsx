...
export default function App() {
  const [allItems, setAllItems] = useState<FileItem[]>(MOCK_DATA);
  ...
  const handleDisconnectDropbox = useCallback(() => {
      if (dropboxClientRef.current) {
          dropboxClientRef.current.auth.tokenRevoke();
      }
  ...
  const renderContent = () => {
    ...
    switch(activePage) {
      case 'home': return <FileExplorer viewMode={viewMode} items={displayedItems} path={path} onNavigate={handleNavigate} {...commonProps} />;
  ...
  return (
    <div className="flex h-screen bg-[#F7F9FC] font-sans overflow-hidden">
      ...
      <Sidebar activePage={activePage} onPageChange={handleSetPage} userRole={currentUser.role} isOpen={isSidebarOpen} onClose={() => setSidebarOpen(false)} isCloudConnected={!!activeCloud} />
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header viewMode={viewMode} setViewMode={setViewMode} user={currentUser} onLogout={handleLogout} onCreateFolder={handleCreateFolder} onUpload={handleUploadClick} onToggleSidebar={() => setSidebarOpen(!isSidebarOpen)} />
        <main className="flex-1 flex overflow-hidden">
          <div className="flex-1 p-4 md:p-8 overflow-y-auto">{renderContent()}</div>
          {selectedItem && <FileDetailPanel item={selectedItem} onClose={() => setSelectedItem(null)} currentUser={currentUser} onPreviewClick={setFullScreenPreviewItem} />}
        </main>
      </div>
